...
parseProcs = lines >>> drop 10 >>> map
    (\l -> let (a, b, c) = (nextNum l, nextNum a, nextNum b) in
        Proc { num = int a, from = int b - 1, to = int c - 1 })
  where
    nextNum s | isDigit (head s) = nextNum (dropWhile isDigit s)
